...
fun getMiddleCharacters(string: String): String {
    val theString = if (string.length in 1..100) {
        string
    } else {
        ""
    }
    lateinit var hasil: String
    hasil = if (theString.length % 2 == 0) {
        "${theString[(theString.count() / 2) - 1]}${theString[theString.count() / 2]}"
    } else {
        "${theString[theString.count() / 2]}"
    }
    return hasil
}
