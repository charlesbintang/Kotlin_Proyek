...
fun minAndMax(number: Int): Int {
    val listChar: MutableList<Char> = mutableListOf()
    number.toString().forEach { listChar.add(it) }
    val valueA = listChar.apply { this.size in 1..8 }.sorted().take(1)
    val valueB = listChar.apply { this.size in 1..8 }.sorted().takeLast(1)
    val finalValue = valueA + valueB
    return finalValue.sumOf { it.toString().toInt() }
}
